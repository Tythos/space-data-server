...
import packageJSONDataStandards from "../../packages/spacedatastandards.org/package.json" assert { type: "json" };

let standardsFile = existsSync("./config.json")
  ? "./config.json"
  : "./lib/config/default.config.json";

console.log(standardsFile);

const config = JSON.parse(readFileSync(standardsFile));
...
writeFileSync(`${typeGenPath}/index.d.ts`, "");
...
Promise.all(generatePromises).then(() => {
  ...
  for (let f = 0; f < folders.length; f++) {
    ...
    let mainIDL = readFileSync(`${packageJSON.standardsPath}/${folderName}/main.fbs`,
      en);
    _schemas[folderName] = mainSchema;
    _idl[folderName] = mainIDL;
  ...
  writeFileSync(
    "./lib/standards/idl.json",
    JSON.stringify(_idl, null, 4)
  );

  writeFileSync("./lib/standards/standards.ts", _src);
});

databaseConfig.connection.filename = databaseConfig.connection.filename.replace(
  basename(databaseConfig.connection.filename),
  `spacedatastandards.${packageJSONDataStandards.version}.sqlite`
);

databaseConfig.version = packageJSONDataStandards.version;
config.database.config[config.database.config.primary] = databaseConfig;

writeFileSync(standardsFile, JSON.stringify(config, null, 4));
writeFileSync(
  "./test/core.test/config/test.database.config.json",
  JSON.stringify(databaseConfig, null, 4)
);
