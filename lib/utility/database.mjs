...
const buildTable = (namespace) => {
  ...
  for (let rootPredicate in nJSONSchema) {
    tSchema.createTable(rootPredicate, function (table) {
      ...
      for (let predicate in nJSONSchema[rootPredicate]) {
        const _predicate = nJSONSchema[rootPredicate][predicate];
      ...
      console.log(
        rootPredicate,
        JSON.stringify(foreignKeys[rootPredicate], null, 4)
      );
      if (
        foreignKeys[rootPredicate] &&
        Object.keys(foreignKeys[rootPredicate])
      ) {
        for (let fProperty in foreignKeys[rootPredicate]) {
          console.log(fProperty, foreignKeys[rootPredicate][fProperty]);
        }
      }
      /* if (intermediateForeignKey[rootPredicate]) {
        for (let fpredicate in intermediateForeignKey[rootPredicate]) {
          console.log(intermediateForeignKey[rootPredicate][fpredicate]);
          const { type, tableName } =
            intermediateForeignKey[rootPredicate][fpredicate];
