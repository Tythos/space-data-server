...
const buildTable = (
  ...
  console.log(rootPredicate);
  tSchema.createTable(rootPredicate, async function (table) {
    table.increments();
    table.timestamps();
    ...
    for (let predicate in intermediateJSON) {
      if (intermediateJSON[predicate].type !== "object")
        console.log(rootPredicate, predicate, intermediateJSON[predicate]);
    }
    if (parentTable) {
      table
        .integer("id")
        .notNullable()
        .references(rootPredicate)
        .inTable(parentTable)
        .onDelete("CASCADE");
    }
    if (foreignJSON) {
