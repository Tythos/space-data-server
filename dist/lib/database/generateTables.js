...
export const fTCheck = (ftPredicate) => ~foreignTypes.indexOf(ftPredicate);
...
const builder = (predicateName, predicate, jsonSchema, rootPredicate, parentPredicate) => {
    ...
    if ($ref) {
        return builder(predicateName, resolver(predicate, jsonSchema), jsonSchema, rootPredicate);
    }
    else if (type === "object") {
...
export const generateDatabase = (jsonSchemas, filename = ".tmp/standards.sqlite", sqlFilename = "", knexConnection) => {
    ...
    return tSchema.then(() => {
        if (sqlFilename.length) {
            writeFileSync(sqlFilename, tSchema
                .toString()
                .replaceAll(",", ",\n")
                .replaceAll(";", ";\n\n")
                .replaceAll("(", "(\n "));
