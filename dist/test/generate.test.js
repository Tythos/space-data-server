...
describe('Test Data Entry', () => {
    ...
    const buildObject = (classProperties, parentClass, tableName, jsonSchema, topLevel = false) => {
            ...
            if (!fTCheck(resolvedProp?.type)) {
                ...
                for (let i = 0; i < 2; i++) {
                    let aObject = !fTCheck(resolvedProp?.type) ?
                        ...
                        buildObject(resolvedProp?.items || resolvedProp.properties, parentClass, refRootName(resolvedProp.$$ref), jsonSchema);
